...
COMMENT_OVERRIDES = {
    ("LIT", "Theme 1 · Deep tech is ecosystem-dependent"): [
        "The primary study finds that specific deep-tech knowledge and talent matter more for startup emergence than generic ecosystem quality or formal institutions.",
        "Its regional analysis covers 272 European NUTS-2 regions and 1,605 deep-tech startups.",
        "→ For Switzerland, specialized research infrastructure and talent concentration are strategic assets.",
    ],
    ("LIT", "Theme 2 · TTOs: relationships, not transactions"): [
        "Chen et al. describe TTOs as actors that translate science into commercially and socially valuable outcomes.",
        "The playmaker role is relational: TTOs build relationships across the wider university innovation-service ecosystem.",
        "→ PSI's TTO should be designed as an active orchestrator, not only a transaction desk.",
    ],
    ("DC", "Data · TTO survival: p=0.0034"): [
        "The primary case study describes TTOs as 'playmakers' whose influence grows through relational ties and access to the wider innovation-service ecosystem.",
        "The key mechanism visible in the source is relationship-building, not a narrow technology-transfer transaction.",
        "→ The interview phase should test whether Swiss TTOs operate this way in practice.",
    ],
}
...
def finding(tag, key, title, old_ref, old_comments, page_no):
    ...
    for c in comments:
        ...
        text(s, rx + 0.28, cy, rw - 0.28, 0.72, [(c, 11, False, BODY, 7)])
        cy += 0.82
...
out = Path(r"G:\Mon Drive\ZHermes Agent\Bachelor Thesis\presentations\Milestone1_Findings_Deck.pptx")
