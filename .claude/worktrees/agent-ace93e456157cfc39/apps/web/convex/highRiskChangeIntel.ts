/**
 * WS-57 — Security Hotspot Change Detector: Convex entrypoints.
 *
 * Analyses the list of changed file paths from a push event for modifications
 * to security-critical code areas: authentication, cryptographic primitives,
 * payment processing, administration panels, PII handlers, and security
 * middleware.
 *
 * Triggered fire-and-forget from events.ts on every push.
 *
 * Entrypoints:
 *   recordHighRiskChangeScan           — internalMutation: run scanner, persist result
 *   triggerHighRiskChangeScan          — public mutation: on-demand by slug+fullName
 *   getLatestHighRiskChangeScan        — public query: most recent result for a repo
 *   getLatestHighRiskChangeScanBySlug  — public query: slug-based (for dashboard/HTTP)
 *   getHighRiskChangeScanHistory       — public query: last 30 lean summaries
 *   getHighRiskChangeSummaryByTenant   — public query: tenant-wide aggregate
 */
import { v } from 'convex/values'
import { internalMutation, mutation, query } from './_generated/server'
import { detectHighRiskChanges } from './lib/highRiskChangeDetector'
...
// ---------------------------------------------------------------------------
// recordHighRiskChangeScan — internalMutation
// ---------------------------------------------------------------------------

export const recordHighRiskChangeScan = internalMutation({
  args: {
  ...
  handler: async (ctx, { tenantId, repositoryId, commitSha, branch, changedFiles }) => {
    ...
    const result = detectHighRiskChanges(paths)
    const now = Date.now()

    await ctx.db.insert('highRiskChangeResults', {
      tenantId,
      repositoryId,
      commitSha,
      branch,
...
// ---------------------------------------------------------------------------
// triggerHighRiskChangeScan — public mutation
// ---------------------------------------------------------------------------

export const triggerHighRiskChangeScan = mutation({
  args: {
    tenantSlug: v.string(),
...
// ---------------------------------------------------------------------------
// getLatestHighRiskChangeScan — public query
// ---------------------------------------------------------------------------

export const getLatestHighRiskChangeScan = query({
  args: {
    repositoryId: v.id('repositories'),
  },
...
// ---------------------------------------------------------------------------
// getLatestHighRiskChangeScanBySlug — public query (slug-based, for dashboard)
// ---------------------------------------------------------------------------

export const getLatestHighRiskChangeScanBySlug = query({
  args: {
    tenantSlug: v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getHighRiskChangeScanHistory — public query (lean)
// ---------------------------------------------------------------------------

export const getHighRiskChangeScanHistory = query({
  args: {
    repositoryId: v.id('repositories'),
...
// ---------------------------------------------------------------------------
// getHighRiskChangeSummaryByTenant — public query
// ---------------------------------------------------------------------------

export const getHighRiskChangeSummaryByTenant = query({
  args: {
    tenantId: v.id('tenants'),
  },
