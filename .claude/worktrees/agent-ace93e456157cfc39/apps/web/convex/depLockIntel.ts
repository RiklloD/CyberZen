/**
 * WS-58 — Dependency Lock File Integrity Verifier: Convex entrypoints.
 *
 * Analyses the changed file paths from a push event for dependency lock-file
 * integrity violations: direct lock edits without manifests, conflicting npm
 * lock formats, and manifests updated without their corresponding locks.
 *
 * Triggered fire-and-forget from events.ts on every push.
 *
 * Entrypoints:
 *   recordDepLockVerifyScan         — internalMutation: run scanner, persist result
 *   triggerDepLockVerifyScan        — public mutation: on-demand by slug+fullName
 *   getLatestDepLockVerifyScan      — public query: most recent result for a repo
 *   getLatestDepLockVerifyScanBySlug — public query: slug-based (for dashboard/HTTP)
 *   getDepLockVerifyScanHistory     — public query: last 30 lean summaries
 *   getDepLockVerifySummaryByTenant — public query: tenant-wide aggregate
 */
import { v } from 'convex/values'
import { internalMutation, mutation, query } from './_generated/server'
import { verifyDepLockIntegrity } from './lib/depLockVerifier'
...
// ---------------------------------------------------------------------------
// recordDepLockVerifyScan — internalMutation
// ---------------------------------------------------------------------------

export const recordDepLockVerifyScan = internalMutation({
  args: {
  ...
  handler: async (ctx, { tenantId, repositoryId, commitSha, branch, changedFiles }) => {
    ...
    const result = verifyDepLockIntegrity(paths)
    const now = Date.now()

    await ctx.db.insert('depLockVerifyResults', {
      tenantId,
      repositoryId,
      commitSha,
      branch,
...
// ---------------------------------------------------------------------------
// triggerDepLockVerifyScan — public mutation
// ---------------------------------------------------------------------------

export const triggerDepLockVerifyScan = mutation({
  args: {
    tenantSlug: v.string(),
...
// ---------------------------------------------------------------------------
// getLatestDepLockVerifyScan — public query
// ---------------------------------------------------------------------------

export const getLatestDepLockVerifyScan = query({
  args: {
    repositoryId: v.id('repositories'),
  },
...
// ---------------------------------------------------------------------------
// getLatestDepLockVerifyScanBySlug — public query (slug-based, for dashboard)
// ---------------------------------------------------------------------------

export const getLatestDepLockVerifyScanBySlug = query({
  args: {
    tenantSlug: v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getDepLockVerifyScanHistory — public query (lean summaries)
// ---------------------------------------------------------------------------

export const getDepLockVerifyScanHistory = query({
  args: {
    repositoryId: v.id('repositories'),
...
// ---------------------------------------------------------------------------
// getDepLockVerifySummaryByTenant — public query
// ---------------------------------------------------------------------------

export const getDepLockVerifySummaryByTenant = query({
  args: {
    tenantId: v.id('tenants'),
  },
