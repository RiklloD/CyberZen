 ...
 * developer security tooling configuration files. Covers 8 domains:
 * secret scanning (gitleaks/trufflehog), SAST (SonarQube/Semgrep/Bandit),
 * SCA (Snyk/ORT/Dependency-Check), security linting (Brakeman/SpotBugs),
 * DAST (ZAP/Burp/Nikto/Nuclei), license policy (FOSSA/license-finder),
 * container scanning (Trivy/Grype/Anchore), and security baselines
 * (Talisman/Hadolint/Safety/MegaLinter).
 *
 * Triggered fire-and-forget from events.ts on every push.
 *
 * Entrypoints:
 *   recordDevSecToolsDriftScan        — internalMutation: run scanner, persist result
 *   triggerDevSecToolsDriftScan       — public mutation: on-demand by slug+fullName
 *   getLatestDevSecToolsDriftScan     — public query: most recent result for a repo
 *   getLatestDevSecToolsDriftBySlug   — public query: slug-based (for dashboard/HTTP)
 *   getDevSecToolsDriftScanHistory    — public query: last 30 lean summaries
 *   getDevSecToolsDriftSummaryByTenant — public query: tenant-wide aggregate
 */
import { v } from 'convex/values'
import { internalMutation, mutation, query } from './_generated/server'
import { scanDevSecToolsDrift } from './lib/devSecToolsDrift'

const MAX_ROWS_PER_REPO = 30
const MAX_PATHS_PER_SCAN = 500

// ---------------------------------------------------------------------------
// recordDevSecToolsDriftScan — internalMutation
// ---------------------------------------------------------------------------

export const recordDevSecToolsDriftScan = internalMutation({
  args: {
  ...
  handler: async (ctx, { tenantId, repositoryId, commitSha, branch, changedFiles }) => {
    ...
    const result = scanDevSecToolsDrift(paths)
    const now    = Date.now()

    await ctx.db.insert('devSecToolsDriftResults', {
      tenantId,
      repositoryId,
      commitSha,
      branch,
...
// ---------------------------------------------------------------------------
// triggerDevSecToolsDriftScan — public mutation
// ---------------------------------------------------------------------------

export const triggerDevSecToolsDriftScan = mutation({
  args: {
    tenantSlug:         v.string(),
...
// ---------------------------------------------------------------------------
// getLatestDevSecToolsDriftScan — public query
// ---------------------------------------------------------------------------

export const getLatestDevSecToolsDriftScan = query({
  args: {
    repositoryId: v.id('repositories'),
  },
...
// ---------------------------------------------------------------------------
// getLatestDevSecToolsDriftBySlug — public query (slug-based, for dashboard)
// ---------------------------------------------------------------------------

export const getLatestDevSecToolsDriftBySlug = query({
  args: {
    tenantSlug:         v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getDevSecToolsDriftScanHistory — public query (lean summaries)
// ---------------------------------------------------------------------------

export const getDevSecToolsDriftScanHistory = query({
  args: {
    repositoryId: v.id('repositories'),
...
// ---------------------------------------------------------------------------
// getDevSecToolsDriftSummaryByTenant — public query
// ---------------------------------------------------------------------------

export const getDevSecToolsDriftSummaryByTenant = query({
  args: {
    tenantId: v.id('tenants'),
  },
