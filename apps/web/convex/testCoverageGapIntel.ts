/**
 * WS-61 — Test Coverage Gap Detector for Security-Critical Code: Convex entrypoints.
 *
 * Analyses the changed file paths from a push event to detect security-critical
 * source file modifications that lack corresponding test coverage changes in
 * the same commit. Covers 6 domains: auth, crypto, payment, authz, session,
 * and security middleware.
 *
 * Triggered fire-and-forget from events.ts on every push.
 *
 * Entrypoints:
 *   recordTestCoverageGapScan        — internalMutation: run scanner, persist result
 *   triggerTestCoverageGapScan       — public mutation: on-demand by slug+fullName
 *   getLatestTestCoverageGapScan     — public query: most recent result for a repo
 *   getLatestTestCoverageGapBySlug   — public query: slug-based (for dashboard/HTTP)
 *   getTestCoverageGapScanHistory    — public query: last 30 lean summaries
 *   getTestCoverageGapSummaryByTenant — public query: tenant-wide aggregate
 */
import { v } from 'convex/values'
import { internalMutation, mutation, query } from './_generated/server'
import { scanTestCoverageGaps } from './lib/testCoverageGap'
...
// ---------------------------------------------------------------------------
// recordTestCoverageGapScan — internalMutation
// ---------------------------------------------------------------------------

export const recordTestCoverageGapScan = internalMutation({
  args: {
  ...
  handler: async (ctx, { tenantId, repositoryId, commitSha, branch, changedFiles }) => {
    ...
    const result = scanTestCoverageGaps(paths)
    const now = Date.now()

    await ctx.db.insert('testCoverageGapResults', {
      tenantId,
      repositoryId,
      commitSha,
      branch,
...
// ---------------------------------------------------------------------------
// triggerTestCoverageGapScan — public mutation
// ---------------------------------------------------------------------------

export const triggerTestCoverageGapScan = mutation({
  args: {
    tenantSlug: v.string(),
...
// ---------------------------------------------------------------------------
// getLatestTestCoverageGapScan — public query
// ---------------------------------------------------------------------------

export const getLatestTestCoverageGapScan = query({
  args: {
    repositoryId: v.id('repositories'),
  },
...
// ---------------------------------------------------------------------------
// getLatestTestCoverageGapBySlug — public query (slug-based, for dashboard)
// ---------------------------------------------------------------------------

export const getLatestTestCoverageGapBySlug = query({
  args: {
    tenantSlug: v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getTestCoverageGapScanHistory — public query (lean summaries)
// ---------------------------------------------------------------------------

export const getTestCoverageGapScanHistory = query({
  args: {
    repositoryId: v.id('repositories'),
...
// ---------------------------------------------------------------------------
// getTestCoverageGapSummaryByTenant — public query
// ---------------------------------------------------------------------------

export const getTestCoverageGapSummaryByTenant = query({
  args: {
    tenantId: v.id('tenants'),
  },
