 ...
 * network-level firewall and access-control configuration files. Covers
 * 8 domains: iptables/ip6tables rules, nftables config, HAProxy ACL config,
 * UFW rules, VPN (WireGuard/OpenVPN) configs, DNS/BIND security config,
 * proxy access control (Squid/nginx ACL), and firewalld zone config.
 *
 * Triggered fire-and-forget from events.ts on every push.
 *
 * Entrypoints:
 *   recordNetworkFirewallDriftScan        — internalMutation: run scanner, persist result
 *   triggerNetworkFirewallDriftScan       — public mutation: on-demand by slug+fullName
 *   getLatestNetworkFirewallDriftScan     — public query: most recent result for a repo
 *   getLatestNetworkFirewallDriftBySlug   — public query: slug-based (for dashboard/HTTP)
 *   getNetworkFirewallDriftScanHistory    — public query: last 30 lean summaries
 *   getNetworkFirewallDriftSummaryByTenant — public query: tenant-wide aggregate
 */
import { v } from 'convex/values'
import { internalMutation, mutation, query } from './_generated/server'
import { scanNetworkFirewallDrift } from './lib/networkFirewallDrift'

const MAX_ROWS_PER_REPO = 30
const MAX_PATHS_PER_SCAN = 500

// ---------------------------------------------------------------------------
// recordNetworkFirewallDriftScan — internalMutation
// ---------------------------------------------------------------------------

export const recordNetworkFirewallDriftScan = internalMutation({
  args: {
  ...
  handler: async (ctx, { tenantId, repositoryId, commitSha, branch, changedFiles }) => {
    ...
    const result = scanNetworkFirewallDrift(paths)
    const now    = Date.now()

    await ctx.db.insert('networkFirewallDriftResults', {
      tenantId,
      repositoryId,
      commitSha,
      branch,
    ...
    const all = await ctx.db
      .query('networkFirewallDriftResults')
...
// ---------------------------------------------------------------------------
// triggerNetworkFirewallDriftScan — public mutation
// ---------------------------------------------------------------------------

export const triggerNetworkFirewallDriftScan = mutation({
  args: {
    tenantSlug:         v.string(),
...
// ---------------------------------------------------------------------------
// getLatestNetworkFirewallDriftScan — public query
// ---------------------------------------------------------------------------

export const getLatestNetworkFirewallDriftScan = query({
  args: {
    repositoryId: v.id('repositories'),
  },
...
// ---------------------------------------------------------------------------
// getLatestNetworkFirewallDriftBySlug — public query (slug-based, for dashboard)
// ---------------------------------------------------------------------------

export const getLatestNetworkFirewallDriftBySlug = query({
  args: {
    tenantSlug:         v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getNetworkFirewallDriftScanHistory — public query (lean summaries)
// ---------------------------------------------------------------------------

export const getNetworkFirewallDriftScanHistory = query({
  args: {
    repositoryId: v.id('repositories'),
...
// ---------------------------------------------------------------------------
// getNetworkFirewallDriftSummaryByTenant — public query
// ---------------------------------------------------------------------------

export const getNetworkFirewallDriftSummaryByTenant = query({
  args: {
    tenantId: v.id('tenants'),
  },
