// WS-33 — Infrastructure as Code (IaC) Security Scanner: Convex entrypoints.
//
//   recordIacScan               — internal mutation: scans a list of
//       {filename, content} items and stores the combined result.
//
//   triggerIacScanForRepository — public mutation: on-demand / webhook trigger.
//
//   getLatestIacScan            — public query: latest result for a repo.
//
//   getIacScanHistory           — public query: lean last-N results for sparklines.
//
//   getIacScanSummaryByTenant   — public query: tenant-wide aggregate.

import { v } from 'convex/values'
import { internal } from './_generated/api'
import { internalMutation, mutation, query } from './_generated/server'
import { combineIacResults, scanIacFile } from './lib/iacSecurity'
...
// ---------------------------------------------------------------------------
// recordIacScan (internal mutation)
// ---------------------------------------------------------------------------

export const recordIacScan = internalMutation({
  args: {
    tenantId: v.id('tenants'),
    repositoryId: v.id('repositories'),
    branch: v.string(),
    commitSha: v.optional(v.string()),
    /** IaC file items to scan: filename + full file content. */
    fileItems: v.array(
      v.object({
        filename: v.string(),
        content: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const fileResults = args.fileItems
      ...
      .map((item) => scanIacFile(item.filename, item.content))

    const combined = combineIacResults(fileResults)
    ...
    const now = Date.now()
    await ctx.db.insert('iacScanResults', {
...
// ---------------------------------------------------------------------------
// triggerIacScanForRepository (public mutation)
// ---------------------------------------------------------------------------

export const triggerIacScanForRepository = mutation({
  args: {
    tenantSlug: v.string(),
...
export const getLatestIacScan = query({
  args: {
    tenantSlug: v.string(),
    repositoryFullName: v.string(),
  },
...
// ---------------------------------------------------------------------------
// getIacScanHistory (public query)
// ---------------------------------------------------------------------------

export const getIacScanHistory = query({
  args: {
    tenantSlug: v.string(),
    repositoryFullName: v.string(),
...
// ---------------------------------------------------------------------------
// getIacScanSummaryByTenant (public query)
// ---------------------------------------------------------------------------

export const getIacScanSummaryByTenant = query({
  args: {
    tenantSlug: v.string(),
  },
  handler: async (ctx, args) => {
