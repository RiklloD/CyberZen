  ...
  handler: async (ctx, { tenantSlug, authToken, severity, status }) => {
    ...
    const rows = findings.map((f: any) => {
      ...
      return [
        ...
        csvEscape((f.affectedPackages ?? []).join('; ')),
...
export const exportExecReportPdf = action({
  ...
  handler: async (ctx, { tenantSlug }) => {
    ...
    const worstRows = (report.worstRepos ?? [])
      .map(
        (r: any) =>
          `<tr><td>${r.fullName ?? r.repositoryId ?? '—'}</td><td>${r.score ?? '—'}</td></tr>`,
      )
      .join('\n')
...
<head>
<meta charset="UTF-8">
<title>CyberZen Executive Report — ${tenantSlug}</title>
<style>
...
<p><strong>Tenant:</strong> ${tenantSlug} &nbsp;|&nbsp; <strong>Generated:</strong> ${new Date(report.generatedAt).toLocaleDateString()}</p>

<h2>Key Performance Indicators</h2>
<div>
  <div class="kpi"><div class="value">${report.openCritical ?? '—'}</div><div class="label">Open Critical</div></div>
  <div class="kpi"><div class="value">${report.mttr ?? '—'}</div><div class="label">MTTR</div></div>
  <div class="kpi"><div class="value">${((report.gateBlockRate ?? 0) * 100).toFixed(1)}%</div><div class="label">Gate Block Rate</div></div>
</div>

<h2>Domain Averages</h2>
<table>
  <tr><th>Domain</th><th>Score (0–100)</th></tr>
  <tr><td>Health</td><td>${report.domainAverages?.healthAvg ?? '—'}</td></tr>
  <tr><td>Drift Posture</td><td>${report.domainAverages?.driftPostureAvg ?? '—'}</td></tr>
  <tr><td>Supply Chain</td><td>${report.domainAverages?.supplyChainAvg ?? '—'}</td></tr>
  <tr><td>Compliance</td><td>${report.domainAverages?.complianceAvg ?? '—'}</td></tr>
</table>

${worstRows ? `<h2>Top At-Risk Repositories</h2><table><tr><th>Repository</th><th>Score</th></tr>${worstRows}</table>` : ''}

<div class="footer"><p>Generated by CyberZen Sentinel · ${ts}</p></div>
</body>
</html>`
...
export const exportComplianceEvidencePdf = action({
  ...
  handler: async (ctx, { tenantSlug, framework }) => {
    ...
    const rows = items
      .map(
        (e: any) =>
          `<tr><td>${e.framework ?? '—'}</td><td>${e.controlId ?? '—'}</td><td>${e.status ?? '—'}</td><td>${e.collectedAt ? new Date(e.collectedAt).toLocaleDateString() : '—'}</td></tr>`,
      )
      .join('\n')
...
<head>
<meta charset="UTF-8">
<title>CyberZen Compliance Evidence — ${tenantSlug}</title>
<style>
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 800px; margin: 0 auto; padding: 2rem; color: #1a1a2e; }
  h1 { font-size: 1.5rem; border-bottom: 2px solid #0077b6; padding-bottom: 0.5rem; }
  table { width: 100%; border-collapse: collapse; margin: 1rem 0; font-size: 0.85rem; }
...
<p><strong>Tenant:</strong> ${tenantSlug} &nbsp;|&nbsp; <strong>Generated:</strong> ${ts}${framework ? ` &nbsp;|&nbsp; <strong>Framework:</strong> ${framework}` : ''}</p>
