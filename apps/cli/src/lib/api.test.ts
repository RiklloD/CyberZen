...
describe('HTTP client', () => {
  ...
  test('maps 401 to AuthError and 429 to ApiError', async () => {
    setup()
    globalThis.fetch = mock(async () => new Response(JSON.stringify({ error: 'bad key' }), { status: 401 })) as typeof fetch
    await expect(api({ path: '/api/findings' })).rejects.toMatchObject({ exitCode: 2 })
    globalThis.fetch = mock(async () => new Response('slow down', { status: 429, headers: { 'Retry-After': '4' } })) as typeof fetch
