...
describe('HTTP client', () => {
  test('sends bearer auth and query parameters', async () => {
    ...
    await expect(api({ path: '/api/findings', query: { tenant: 'acme', limit: 1 } })).resolves.toEqual({ ok: true })
  ...
  test('maps 401 to AuthError and 429 to ApiError', async () => {
    setup()
    globalThis.fetch = mock(async () => new Response(JSON.stringify({ error: 'bad key' }), { status: 401 })) as unknown as typeof fetch
    await expect(api({ path: '/api/findings' })).rejects.toMatchObject({ exitCode: 2 })
    globalThis.fetch = mock(async () => new Response('slow down', { status: 429, headers: { 'Retry-After': '4' } })) as unknown as typeof fetch
    await expect(api({ path: '/api/findings' })).rejects.toMatchObject({ status: 429, hint: 'Retry after 4 seconds.' })
  })

  test('redacts credentials from diagnostics', () => {
    expect(redactHeaders({ Authorization: 'Bearer secret', 'X-MSSP-Api-Key': 'msk_secret', Accept: 'json' })).toEqual({
