...
import { readAuth, siteUrl, writeAuth, type AuthFile } from './config'
...
export async function deviceLogin(options: DeviceLoginOptions = {}): Promise<DeviceLoginResult> {
  ...
  if (!start.ok) throw new AuthError(`Device login could not start (HTTP ${start.status}).`)
  ...
  if (options.openBrowser !== false) {
    try { await open(challenge.verificationUrl) } catch { /* print URL is sufficient */ }
  }

  const deadline = Date.now() + (options.timeoutMs ?? challenge.expiresIn * 1000)
  while (Date.now() < deadline) {
    await new Promise((resolve) => setTimeout(resolve, challenge.interval * 1000))
    ...
    if (!poll.ok) throw new AuthError(`Device login polling failed (HTTP ${poll.status}).`)
    const result = await poll.json() as { status: string; token?: string; tenantSlug?: string }
    ...
    if (result.status === 'denied' || result.status === 'expired') throw new AuthError(`Device login ${result.status}.`)
  }
  throw new UsageError('Device login timed out.', 'Run `cyberzen auth login` again.')
}
