...
import { findProjectPath, linkProject, readProject, resolveTenant, unlinkProject } from './project'
...
describe('project linkage', () => {
  test('writes and reads a directory link', () => {
    const path = linkProject({ tenantSlug: 'acme', repoFullName: 'acme/app' }, root)
    expect(path).toContain('.cyberzen')
    expect(readProject(root)).toMatchObject({ tenantSlug: 'acme', repoFullName: 'acme/app' })
