...
export function linkProject(link: Omit<ProjectLink, 'linkedAt'>, directory = process.cwd()): string {
  const path = projectPath(directory)
  mkdirSync(dirname(path), { recursive: true })
  writeFileSync(path, `${JSON.stringify({ ...link, linkedAt: Date.now() }, null, 2)}\n`, 'utf8')
  return path
}
...
export function resolveTenant(explicit?: string, directory = process.cwd()): string | undefined {
  return explicit ?? process.env.CYBERZEN_TENANT ?? readProject(directory)?.tenantSlug
}
