...
function buildUrl(base: string, path: string, query?: ApiOptions['query']): string {
...
export async function api<T = unknown>(options: ApiOptions): Promise<T> {
  ...
  const url = buildUrl(options.baseUrl ?? siteUrl(), options.path, options.query)
  ...
  try {
    ...
    if (response.status === 401) {
      throw new AuthError(typeof payload === 'object' && payload !== null && 'error' in payload
        ? String((payload as { error: unknown }).error)
        : 'API key was rejected.')
    }
    if (response.status === 429) {
      const retryAfter = response.headers.get('retry-after')
      throw new ApiError(response.status, url, formatBody(payload), retryAfter ? `Retry after ${retryAfter} seconds.` : undefined)
    }
    if (!response.ok) throw new ApiError(response.status, url, formatBody(payload))
    return payload as T
  } catch (error) {
