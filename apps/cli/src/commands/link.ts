...
export function registerLink(program: Command): void {
	const link = program
		.command("link")
		.description("Link the current directory to a CyberZen tenant");
	link
		.option("--tenant <slug>", "Tenant slug")
		.option("--repo <owner/name>", "Default repository")
	...
	program
		.command("link:status", { hidden: true })
		.action((_options: unknown, command: Command) => {
			const globals = globalsOf(command);
			const project = readProject();
			render({ linked: Boolean(project), ...project }, globals);
