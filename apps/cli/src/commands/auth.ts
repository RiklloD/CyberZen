...
export function registerAuth(program: Command): void {
  const auth = program.command('auth').description('Manage CyberZen authentication')
  ...
  auth.command('login')
    ...
    .action(async (options: { token?: string; tenant?: string; email?: string }, command: Command) => {
      ...
      await api({ path: '/api/findings', query: { limit: 1 }, token, timeout: globals.timeout })
      saveToken(token, { tenantSlug: options.tenant ?? globals.tenant, email: options.email })
      render({ authenticated: true, token: tokenPreview(token), tenant: options.tenant ?? globals.tenant }, globals)
    })
